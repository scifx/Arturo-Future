        ...
        if (irec.EventType == KEY_EVENT) {
            ...
            if (k->bKeyDown || k->wVirtualKeyCode == VK_MENU) {
                ...
                /* Note that control characters are already translated in AsciiChar */
                    ...
                    if (wc >= UNI_SUR_HIGH_START && wc <= UNI_SUR_HIGH_END) {
                        INPUT_RECORD irec2;
                        DWORD n2;
                        if (WaitForSingleObject(current->inh, INFINITE) != WAIT_OBJECT_0) {
                            return wc;
                        ...
                        if (irec2.EventType == KEY_EVENT) {
                            ...
                            WCHAR wc2 = k2->uChar.UnicodeChar;
                            if ((k2->bKeyDown || k2->wVirtualKeyCode == VK_MENU) &&
                                wc2 >= UNI_SUR_LOW_START && wc2 <= UNI_SUR_LOW_END) {
                                return (((int)(wc - UNI_SUR_HIGH_START)) << 10) +
                                       ((int)(wc2 - UNI_SUR_LOW_START)) + 0x10000;
                            }
                        }
                        return wc;
                    }
