...
func appendUtf8Rune(target: var string, source: openArray[char], pos: int) {.inline.} =
    let width = runeLenAt(source, pos)
    for i in 0..<width:
        target.add(source[pos + i])

func isIdentifierStartAt(source: openArray[char], pos: int): bool {.inline.} =
    if pos >= source.len:
        return false

    let ch = source[pos]
    if ch in PermittedIdentifiers_Start:
        return true
    if ord(ch) < 128:
        return false

    var rune: Rune
    fastRuneAt(source, pos, rune, false)
    result = rune.isAlpha or rune.isTitle

func isIdentifierContinueAt(source: openArray[char], pos: int): bool {.inline.} =
    if pos >= source.len:
        return false

    let ch = source[pos]
    if ch in PermittedIdentifiers_In:
        return true
    if ord(ch) < 128:
        return false
    ...
    var rune: Rune
    fastRuneAt(source, pos, rune, false)
    result = rune.isAlpha or rune.isTitle or rune.isCombining
...
template parseIdentifier(p: var Parser, alsoAddCurrent: bool) =
    var pos = p.bufpos
    when alsoAddCurrent:
        if p.buf[pos] in PermittedIdentifiers_Start:
            add(p.value, p.buf[pos])
            inc(pos)
        else:
            appendUtf8Rune(p.value, p.buf, pos)
            inc(pos, runeLenAt(p.buf, pos))
    else:
        inc(pos)
    let initialPos = pos
    while isIdentifierContinueAt(p.buf, pos):
        if p.buf[pos] in PermittedIdentifiers_In:
            add(p.value, p.buf[pos])
            inc(pos)
        else:
            appendUtf8Rune(p.value, p.buf, pos)
...
template parseWordToken(p: var Parser, level: int) =
    ...
    if p.value.len == 1 and p.value[0] == Underscore:
        ...
        if isIdentifierStartAt(p.buf, p.bufpos+1) or 
...
template parsePath(p: var Parser, root: Value, curLevel: int, asLiteral: bool = false) =
    ...
    while p.buf[p.bufpos]==Backslash:
        inc(p.bufpos)
        if isIdentifierStartAt(p.buf, p.bufpos):
            setLen(p.value, 0)
            parseIdentifier(p, alsoAddCurrent=true)
        ...
        case p.buf[p.bufpos]
            ...
            of '\194':
                if p.buf[p.bufpos+1]=='\171': # got «
                    ...
                    inc(p.bufpos, 2)
                elif isIdentifierStartAt(p.buf, p.bufpos):
                    parseWordToken(p, level)
                else:
                    inc(p.bufpos, runeLenAt(p.buf, p.bufpos))
            of '\195':
