...
var
    StoredTranslations : Table[Hash, seq[TranslationCacheEntry]]

#=======================================
# Helpers
#=======================================

func sameCachedValue(x: Value, y: Value): bool

func indexOfValue(a: ValueArray, item: Value): int {.inline.}=
    result = 0
    for i in items(a):
        if item.kind in {Inline, Block, Dictionary, Object, Path, PathLabel, PathLiteral} or
           i.kind in {Inline, Block, Dictionary, Object, Path, PathLabel, PathLiteral}:
            if sameCachedValue(item, i): return
        else:
    ...
    case x.kind:
        of Path, PathLabel, PathLiteral:
            if x.p.len != y.p.len:
                return false
            for i in 0..x.p.high:
                if not sameCachedValue(x.p[i], y.p[i]):
                    return false
            return true
        of Inline, Block:
            if x.a.len != y.a.len:
                return false
            for i in 0..x.a.high:
                if not sameCachedValue(x.a[i], y.a[i]):
                    return false
            return sameCachedValue(x.data, y.data)
        of Dictionary:
            if x.d.len != y.d.len:
                return false
            for k, v in x.d.pairs:
                if not y.d.hasKey(k):
                    return false
                if not sameCachedValue(v, y.d[k]):
                    return false
            return true
        of Object:
            if x.proto != y.proto or x.o.len != y.o.len:
                return false
            for k, v in x.o.pairs:
                if not y.o.hasKey(k):
                    return false
                if not sameCachedValue(v, y.o[k]):
                    return false
            return true
        else:
            return consideredEqual(x, y)

proc getStoredTranslation(root: Value, vhash: Hash, isDictionary: bool, isFunctionBlock: bool, omitNewlines: bool): Translation =
    for entry in StoredTranslations.getOrDefault(vhash, @[]):
        if entry.isDictionary == isDictionary and
           entry.isFunctionBlock == isFunctionBlock and
           entry.omitNewlines == omitNewlines and
           sameCachedValue(entry.root, root):
            return entry.translation

proc storeTranslation(root: Value, vhash: Hash, isDictionary: bool, isFunctionBlock: bool, omitNewlines: bool, translation: Translation) =
    var bucket = StoredTranslations.getOrDefault(vhash, @[])
    bucket.add TranslationCacheEntry(
        root: copyValue(root),
        isDictionary: isDictionary,
        isFunctionBlock: isFunctionBlock,
        omitNewlines: omitNewlines,
        translation: translation
    )
    StoredTranslations[vhash] = bucket
...
template optimizeConditional(
    ...
    if canWeOptimize:
        ...
        if stillProceed:
            ...
            when withLoop and not isSwitch:
        ...
        if item.kind == SpecialCall:
            ...
            case item.op:
                ...
                of opSwitch:    
                    ...
                    optimizeConditional(consts, it, item, isSwitch=true, withInversion=true)
                    when defined(PROFILER):
                        if alreadyProcessed:
...
proc doEvalAndCheckSafety*(root: Value, isDictionary=false, isFunctionBlock=false, omitNewlines=false, useStored: static bool = true): (Translation, bool) {.inline.} = 
    ...
    when useStored:
        if not root.dynamic:
            vhash = hash(root)
            if (let storedTranslation = getStoredTranslation(root, vhash, isDictionary, isFunctionBlock, omitNewlines); not storedTranslation.isNil):
    ...
    when useStored:
        if canStore and (vhash != -1):
            storeTranslation(root, vhash, isDictionary, isFunctionBlock, omitNewlines, res)
        return (res, canStore)
    else:
        return (res, false)
